...
class Logistic_mapping(layers.Layer):
    ...
    def call(self, inputs, logdet=None, reverse=False):
        ...
        if not reverse:
            ...
            if logdet is not None:
                tp =  tf.math.log(self.s/2.0) - tp1 - tp2
            ...
            if logdet is not None:
                x = tf.clip_by_value(x, 1.0e-10, 1.0-1.0e-10)
                tp = tf.math.log(x) + tf.math.log(1-x) + tf.math.log(2.0/self.s)
                dlogdet = tf.reduce_sum(tp, axis=[1], keepdims=True)
                return x, logdet + dlogdet
...
class CDF_quadratic(layers.Layer):
    def __init__(self, name, n_bins, r=1.2, bound=30.0, beta=1e-8, **kwargs):
    ...
    def call(self, inputs, logdet=None, reverse=False):
        with tf.device("cpu:0"):
            ...
            if not reverse:
                ...
                x = tf.where( x > self.bound, self.beta*(x-self.bound)+self.bound, x)
                if logdet is not None:
                    dlognet = x
                    dlogdet = tf.where(dlognet > self.bound, self.beta, 1.0)
                ...
                #for the interval (a,inf)
                x = tf.where( x > self.bound, (x-self.bound)/self.beta+self.bound, x)
                if logdet is not None:
                    dlognet = x
                    dlogdet = tf.where(dlognet > self.bound, 1.0/self.beta, 1.0)
                    dlogdet = tf.reduce_sum(tf.math.log(dlogdet), axis=[1], keepdims=True)
                    logdet += dlogdet
    ...
    def _pdf_normalize(self):
        # peicewise pdf
        p0 = tf.ones((1,self.n_length), dtype=tf.float32)*self.beta
        self.pdf = p0
        px = tf.math.exp(self.p)*(self.elmt_size[:-1]+self.elmt_size[1:])/2.0
        px = (1.0 - (self.elmt_size[0]+self.elmt_size[-1])*self.beta/2.0)/tf.reduce_sum(px, 0, keepdims=True)
    ...
    def _cdf(self, x, logdet=None):
        ...
        k_ind = tf.searchsorted(tf.transpose(xr), tf.transpose(x), side='right')
