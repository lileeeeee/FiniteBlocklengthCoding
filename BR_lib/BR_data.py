...
def gen_nd_Gaussian(ndim, n_train):
    m = n_train
...
def gen_2d_mixture_Gaussian(n_train, r, k, sig):
    m = n_train

    step = 2.0*np.pi/float(k)
    ind = np.random.randint(k, size=(m, 1))
    loc = np.concatenate((r*np.cos(ind*step), r*np.sin(ind*step)), 1)
    drift = np.random.normal(0,1,(m,2))*sig
    x = loc + drift
    return x
...
def log_2d_mixture_Gaussian(x, r, k, sig):
    step = 2.0*np.pi/float(k)
    p = np.zeros_like(x)[:,0].reshape((-1,1))
    loc = np.zeros((1,2))

    for i in range(k):
        loc[0,0] = r*np.cos(float(i)*step) 
        loc[0,1] = r*np.sin(float(i)*step)
        p += np.exp(log_standard_Gaussian(x-loc))
